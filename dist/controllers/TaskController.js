...
class TaskController {
    ...
    static getProjectTasks = async (req, res) => {
        try {
            const tasks = await Task_1.default.find({ project: req.project._id }).populate('project');
            res.json(tasks);
        }
        catch (error) {
            res.status(500).json({ error: 'Hubo un error' });
    ...
    static getTaskById = async (req, res) => {
        try {
            const task = await Task_1.default.findById(req.task._id)
    ...
    static updateTask = async (req, res) => {
        try {
            if (String(req.task.project) !== String(req.project._id)) {
